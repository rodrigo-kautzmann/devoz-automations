...
def render_html(tax: dict) -> str:
    ...
    return (
        ...
        "<p><strong>Quando roda:</strong> automaticamente a cada alteração do "
        "<code>taxonomy.json</code> no <code>main</code>, via GitHub Actions "
        "no <code>devoz-automations</code> · <strong>Automação:</strong> "
        "<code>scripts/taxonomy_confluence_sync.py</code> "
        "(workflow <code>taxonomy-confluence.yml</code>).</p>"
